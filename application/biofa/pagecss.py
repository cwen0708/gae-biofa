...
class init(AdministratorHandler):
    def post(self, *args):
        ...
        for item in list:
            record = db.GqlQuery("SELECT * FROM DBCss WHERE jq_selector = :1", item[1]).get()
            if record is None:
                record = DBCss()
                record.save()
            ...
            record.save()
        self.json({"action": "message","info": u'Css樣式的基本設定已建立 / 復原',"content":u"您已經成功的建立 / 復原基本設定。"})

class list(AdministratorHandler):
    def get(self, *args):
        data_source = db.GqlQuery("SELECT * FROM DBCss ORDER BY sort desc")
        ...
        self.render("/admin/pagecss/list.html")

class create(AdministratorHandler):
    def get(self, *args):
        self.render("/admin/pagecss/create.html")

    def post(self, *args):
        record = DBCss()
        record.title = self.request.get('title') if  self.request.get('title') is not None else u'未命名'
        record.attr = self.request.get('attr') if self.request.get('attr') is not None else u''
        record.value = self.request.get('value') if self.request.get('value') is not None else u''
        record.selector = self.request.get('selector') if self.request.get('selector') is not None else u''
        record.save()
        Pagination.add(record,record.is_enable)
        self.json({"info": u'Css樣式已新增',"content":u"您已經成功的新增了一筆Css樣式。"})
...
class edit(AdministratorHandler):
    def get(self, *args):
        ...
        self.render("/admin/pagecss/edit.html")
    ...
    def post(self, *args):
        ...
        if record is not None:
            ...
            record.selector = self.request.get('selector') if self.request.get('selector') is not None else u''
            record.save()
            self.json({"info": u'Css樣式已更新',"content":u"您已經成功的變更了此筆Css樣式。"})
        else:
